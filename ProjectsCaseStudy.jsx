...
const PROJECTS = [
  ...
  },
  {
    ...
    topics: ["predictive-maintenance", "physics-features", "random-forest"],
    problem: "예지보전 예제는 대개 센서값을 그대로 모델에 넣고 '정확도 99%'로 끝난다. 정작 현장이 궁금한 건 점수가 아니라 '이 설비가 왜 고장 나는가'였다.",
    method: "EDA에서 데이터 누수와 통계적 이상치를 걸러낸 뒤, 센서값을 해석 가능한 물리량(동력, 온도차, 누적응력)으로 바꿨다. 그 피처는 데이터가 아니라 부호 불변식과 손계산 단위 테스트로 검증했다(데이터로 기준을 세우면 순환이라 피함). 고장 조건은 결정트리·Mann-Whitney로 데이터에서 직접 찾아 공개 규칙과 대조했고, 예측은 raw 센서 모델과 물리 피처를 더한 모델을 같은 조건으로 비교했다.",
    result: "물리 피처가 예측을 실제로 끌어올렸다 — RandomForest에서 PR-AUC가 0.761에서 0.889로, F1은 0.496에서 0.874로 올랐다(상호작용을 쓰는 트리에서 특히 컸다). 데이터에서 찾은 고장 조건도 공개 규칙과 거의 맞았다(방열 실패 = 온도차·회전속도가 둘 다 낮은 AND). 점수보다 '왜'를 설명하는 데 초점을 뒀고, 다음은 이걸 정비 판단으로 옮기는 단계다.",
  },
  {
    name: "phm-agent",
